...
DeepSeek Harness 的可扩展媒体引用层。对话日志只保存轻量 `xiao443-media` 块和资源引用，二进制内容留在资源存储中；聊天界面再把引用投影成可预览的媒体卡片。

当前内置了完整的图片链路：

- 选择、拖入和粘贴图片继续使用 Harness 官方输入框交互。
- 发送后把官方草稿图片保存进 `ctx.attachments`，持久化为自定义媒体块。
- 聊天记录使用官方 `ImageGallery`，点击预览效果与原生图片一致。
...
视觉路由需要同时安装 `@dfy-plugins/dsh-vision`。推荐先安装媒体块，再安装视觉插件。
