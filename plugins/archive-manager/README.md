...
按项目列出已归档的对话，并支持取消归档、永久删除单条或全部删除。

- Host 半区：注册列表、取消归档、单条删除和全部删除四个 HTTP 路由，读取 `$DSH_HOME/sessions`、
...
右上角的「全部删除」会在二次确认后永久删除当前全部归档对话，未归档对话会被拒绝。
永久删除会保留不可见的归档标记，防止当前 Host 内存中的旧
